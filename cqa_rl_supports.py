...
def get_selected_example_features(batch_features, batch_example_tracker, batch_variation_tracker,
    ...
    for action, e_tracker, v_tracker in zip(np.squeeze(actions, axis=1), merged_example_tracker,
                                            merged_variation_tracker):
...
def get_state_rep(bert_representations, start_prob_res, end_prob_res, turn_features, tfidf_features, state_features):
    features = [None,
                ...
                np.hstack((np.expand_dims(np.asarray(start_prob_res), axis=-1),
                           np.expand_dims(np.asarray(end_prob_res), axis=-1))),
